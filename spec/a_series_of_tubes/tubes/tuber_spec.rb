...
describe ASeriesOfTubes::Tubes::Tuber do
  ...
  describe '#http_methods' do
    it 'dynamically writes each http method' do
      ...
      ASeriesOfTubes::Tubes::Tuber::METHODS.each do |method|
    ...
    it 'adds a tube when each method is called' do
      tuber = ASeriesOfTubes::Tubes::Tuber.new
      ASeriesOfTubes::Tubes::Tuber::METHODS.each do |method|
        tuber.send method, Regexp.new('^/kovaches$'), ASeriesOfTubes::TubeController, :index
      end
      expect(tuber.tubes.count).to eq ASeriesOfTubes::Tubes::Tuber::METHODS.length
    end
  end
  ...
  describe '#draw' do
    it 'calls http methods with the tube information and adds the tube' do
      ...
      tubes = Proc.new do
        get Regexp.new('^/pea$'), ASeriesOfTubes::TubeController, :show
        post Regexp.new('^/tear$'), ASeriesOfTubes::TubeController, :show
        put Regexp.new('^/gryphon$'), ASeriesOfTubes::TubeController, :show
      end
